...
def load_umap_subcellular(
    path: str | os.PathLike,
    name: str = "czbiohub_subcellular_umap",
    ...
    location_col:
        Column containing subcellular locations/organelles. If None, it will be inferred.
    """
    df = _read_delimited(path)

    # Infer id_col
    if not id_col:
        for col in ["primary_id", "uniprot", "uniprot_id", "accession", "protein_id", "gene", "Gene"]:
    ...
    # Infer location_col
    if not location_col:
        for col in ["organelle", "localization", "location", "cluster", "compartment", "subcellular_location"]:
            if col in df.columns:
                location_col = col
                break
    ...
    if missing:
        raise ValueError(f"Could not infer or find required columns for UMAP subcellular file: {', '.join(missing)}")
    ...
    return AnnotationSet(
        name=name,
        source="CZBIOHUB_SUBCELLULAR_UMAP",
        table=out,
        id_col="primary_id",
        term_col="term_name",
        term_id_col="term_id",
